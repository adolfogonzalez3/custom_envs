...
class MultiOptimize(BaseMultiEnvironment):
    ...
    def __init__(self, data_set='iris', batch_size=None, version=1):
        ...
        self.observation_spaces = {MultiOptimize.AGENT_FMT.format(i):
                                   Box(low=-1e3, high=1e3,
                                       dtype=np.float32,
                                       shape=(state_size,))
                                   for i in range(self.model.size)}
        self.action_spaces = {MultiOptimize.AGENT_FMT.format(i):
                              Box(low=-1e3, high=1e3,
                                  dtype=np.float32,
                                  shape=(1,))
                              for i in range(self.model.size)}
    ...
    def base_reset(self):
        ...
        if self.version == 0:
            ...
            states = {MultiOptimize.AGENT_FMT.format(i): gd
                      for i, gd in enumerate(grad_flat)}
        elif self.version == 1:
            loss_flat = float(self.loss_hist[0])
            grad_flat = self.grad_hist[0].ravel()
            states = {MultiOptimize.AGENT_FMT.format(i): (loss_flat, gd)
                      for i, gd in enumerate(grad_flat)}
        elif self.version == 2:
            wght_flat = self.wght_hist[0].ravel()
            loss_flat = float(self.loss_hist[0])
            grad_flat = self.grad_hist[0].ravel()
            states = {MultiOptimize.AGENT_FMT.format(i): (wght, loss_flat, gd)
                      for i, (wght, gd) in
    ...
    def base_step(self, actions):
        ...
        action = np.reshape([actions[MultiOptimize.AGENT_FMT.format(i)]
