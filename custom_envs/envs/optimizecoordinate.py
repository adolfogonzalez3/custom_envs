...
class OptimizeCorrect(BaseEnvironment):
    ...
    def __init__(self, data_set='mnist', batch_size=None, version=1):
        ...
        for i in range(self.model.size):
            obs_space = Box(low=-1e3, high=1e3, dtype=np.float32,
                            shape=(2*past_history,))
            act_space = Box(low=-1e3, high=1e3, dtype=np.float32,
                            shape=(1,))
            self.observation_space['param_{:d}'.format(i)] = obs_space
            self.action_space['param_{:d}'.format(i)] = obs_space
        self.seed()
    ...
    def base_reset(self):
        ...
        self.sequence.labels = np.roll(self.sequence.labels, 1, axis=1)
        if self.version == 1:
