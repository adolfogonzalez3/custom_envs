...
from custom_envs.wrappers.monitor import Monitor
...
def test_monitor_init(save_path, env_obj):
    ...
    monitor = Monitor(env_obj, Path(save_path) / 'test_init')
    assert str(monitor.file_path).endswith(Monitor.EXT)
    monitor = Monitor(env_obj, None)
...
def test_monitor_save_without_info(save_path, env_obj, step):
    ...
    monitor = Monitor(env_obj, save_file, chunk_size=step+1)
    monitor.reset()
    for _ in range(step):
        _, _, terminal, _ = monitor.step(None)
        if terminal:
            monitor.reset()
    assert not save_file.with_suffix(Monitor.EXT).is_file()
    monitor.save()
    with save_file.with_suffix(Monitor.EXT).open('rt') as csv:
        print('Printing: ', csv.read())
    assert save_file.with_suffix(Monitor.EXT).is_file()
    dataframe = pd.read_csv(save_file.with_suffix(Monitor.EXT))
...
def test_monitor_reset(save_path, env_obj):
    ...
    monitor = Monitor(env_obj, save_file)
...
def test_monitor_step(save_path, env_obj):
    ...
    monitor = Monitor(env_obj, save_file, chunk_size=1,
                      info_keywords=('half',))
    monitor.reset()
